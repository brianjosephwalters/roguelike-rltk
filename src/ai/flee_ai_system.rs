...
impl<'a> System<'a> for FleeAI {
    ...
    fn run(&mut self, data: Self::SystemData) {
        ...
        for (entity, mut pos, flee, mut viewshed, _myturn) in (&entities, &mut positions, &want_flee, &mut viewsheds, &turns).join() {
            ...
            let flee_target = rltk::DijkstraMap::find_highest_exit(&flee_map, my_index, &*map);
            if let Some(flee_target) = flee_target {
                if !crate::spatial::is_blocked(flee_target) {
                    crate::spatial::move_entity(entity, my_index, flee_target);
                    viewshed.dirty = true;
