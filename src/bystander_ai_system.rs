use specs::prelude::*;
use crate::{EntityMoved, Map, RunState, Viewshed, Bystander, Position, Quips, Name};
use rltk::{RandomNumberGenerator, Point};
use specs::{System, WriteExpect, ReadExpect, Entities, WriteStorage, ReadStorage};
    ...
    fn run(&mut self, data: Self::SystemData) {
        ...
        for (entity, mut viewshed, _bystander, mut pos) in (&entities, &mut viewshed, &bystander, &mut position).join() {
            ...
            if x > 0 && x < map.width - 1 && y > 0 && y < map.height - 1 {
                let destination_index = map.xy_index(x, y);
                if !map.blocked[destination_index] {
                    ...
                    map.blocked[destination_index] = true;
                    viewshed.dirty = true;
                }
            }
        }
