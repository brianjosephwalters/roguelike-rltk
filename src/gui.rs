...
use super::{
    Pools, GameLog, Map, Name, Position, Point,
...
pub fn draw_ui(ecs: &World, ctx: &mut Rltk) {
    ...
    ctx.print_color(50,10, white, black, 
...
//         return;
//     }
//     if !map.visible_tiles[map.xy_index(mouse_pos.0, mouse_pos.1)] { return; }
...
fn vendor_sell_menu(gs: &mut State, ctx: &mut Rltk, _vendor: Entity, _mode: VendorMode) 
    ...
    let mut equippable: Vec<Entity> = Vec::new();
    let mut j = 0;
    for (entity, _pack, name, item) 
        in (&entities, &backpack, &names, &items).join()
...
fn vendor_buy_menu(gs: &mut State, ctx: &mut Rltk, vendor: Entity, _mode: VendorMode) 
    -> (VendorResult, Option<Entity>, Option<String>, Option<f32>) {
    use crate::raws::*;
...
pub fn show_vendor_mode(gs: &mut State, ctx: &mut Rltk, vendor: Entity, mode: VendorMode) 
    -> (VendorResult, Option<Entity>, Option<String>, Option<f32>) {
    match mode {
