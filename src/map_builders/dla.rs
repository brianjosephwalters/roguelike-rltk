...
impl DLABuilder {
        ...
        while floor_tile_count < desired_floor_tiles {
            match self.algorithm {
                DLAAlgorithm::WalkInwards => {
                    ...
                    self.paint(previous_x, previous_y);
                }
                // Seems wrong
    ...
    fn paint(&mut self, x: i32, y: i32) {
        ...
        let digger_index = self.map.xy_index(x, y);
        self.map.tiles[digger_index] = TileType::Floor;
    }
                ...
                for brush_y in y - half_brush_size .. y + half_brush_size {
                    for brush_x in x - half_brush_size .. y + half_brush_size {
                        if brush_x > 1 && brush_x < self.map.width - 1 &&
                            ...
                            let index = self.map.xy_index(brush_x, brush_y);
