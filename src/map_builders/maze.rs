...
use specs::World;
use crate::{spawner, SHOW_MAPGEN_VISUALIZER, TileType};
...
impl MazeBuilder {
    ...
    fn build(&mut self) {
        ...
        maze.generate_map(self);

        // Find a starting position, walk left until we find an open position
        ...
        // Find all starting tiles we can reach from the starting position
...
impl<'a> Grid<'a> {
    ...
    fn generate_map(&mut self, generator: &mut MazeBuilder) {
        let mut i = 0;
        loop {
            ...
            match next {
                Some(next) => {
                    ...
                    // split the array at an index and return 2 mutable arrays.
                    // But why split the array at alL?  Why not just get the cells out?
                    // Must be the borrow checker!
    ...
    fn copy_to_map(&self, map: &mut Map) {
        ...
        for cell in self.cells.iter() {
            let x = cell.column + 1;
            let y = cell.row + 1;
            let index = map.xy_index(x * 2, y * 2);

            map.tiles[index] = TileType::Floor;
            if !cell.walls[TOP] { map.tiles[index - map.width as usize] = TileType::Floor }
            if !cell.walls[RIGHT] { map.tiles[index + 1] = TileType::Floor }
            if !cell.walls[BOTTOM] { map.tiles[index + map.width as usize] = TileType::Floor }
            if !cell.walls[LEFT] { map.tiles[index - 1] = TileType::Floor }
        }
    }
}