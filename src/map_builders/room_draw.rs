...
impl MetaMapBuilder for RoomDrawer {
    fn build_map(&mut self, rng: &mut RandomNumberGenerator, builder_data: &mut BuilderMap) {
        self.build(rng, builder_data);
    }
}
    ...
    fn rectangle(&mut self, build_data: &mut BuilderMap, room: &Rect) {
        for y in room.y1 + 1 ..= room.y2 {
            for x in room.x1 + 1 ..= room.x2 {
                ...
                if index > 0 && index < ((build_data.map.width * build_data.map.height) - 1) as usize {
    ...
    fn circle(&mut self, build_data: &mut BuilderMap, room: &Rect) {
        ...
        for y in room.y1 ..= room.y2 {
            for x in room.x1 ..= room.x2 {
                ...
                if index > 0 &&
                    index < ( (build_data.map.width * build_data.map.height) - 1) as usize &&
                    distance <= radius
                {
                    build_data.map.tiles[index] = TileType::Floor;
                }
