...
impl RoomExploder {
    ...
    fn build(&mut self, rng: &mut RandomNumberGenerator, build_data: &mut BuilderMap) {
        ...
        if let Some(room_builder) = &build_data.rooms {
            ...
            panic!("Room Explosions require a build with room structures.");
        }
        ...
        for room in rooms.iter() {
            ...
            if n_diggers > 0 {
                for _i in 0..n_diggers {
                    ...
                    while drunk_life > 0 {
                        let drunk_index = build_data.map.xy_index(drunk_x, drunk_y);
                        if build_data.map.tiles[drunk_index] == TileType::Wall {
                            did_something = true;
                        }
                        paint(&mut build_data.map, Symmetry::None, 1, drunk_x, drunk_y);
                        build_data.map.tiles[drunk_index] = TileType::DownStairs;
