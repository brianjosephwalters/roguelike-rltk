use crate::{Map, Position, spawner, TileType, SHOW_MAPGEN_VISUALIZER};
...
impl VoronoiBuilder {
    ...
    fn build(&mut self) {
        ...
        while voronoi_seeds.len() < n_seeds {
            ...
            let vindex = self.map.xy_index(vx, vy);
            let candidate = (vindex, rltk::Point::new(vx, vy));
        ...
        for y in 1..self.map.height - 1 {
            for x in 1..self.map.width - 1 {
                let mut neighbors = 0;
                let my_index = self.map.xy_index(x, y);
                let my_seed = voronoi_membership[my_index];

                if voronoi_membership[self.map.xy_index(x - 1, y)] != my_seed { neighbors += 1; }
                if voronoi_membership[self.map.xy_index(x + 1, y)] != my_seed { neighbors += 1; }
                if voronoi_membership[self.map.xy_index(x, y - 1)] != my_seed { neighbors += 1; }
                if voronoi_membership[self.map.xy_index(x, y + 1)] != my_seed { neighbors += 1; }

                if neighbors < 2 {
                    self.map.tiles[my_index] = TileType::Floor;
                }
            }
            self.take_snapshot();
        }
