...
impl VoronoiCellBuilder {
    ...
    fn build(&mut self, rng: &mut RandomNumberGenerator, build_data: &mut BuilderMap) {
        ...
        while voronoi_seeds.len() < self.n_seeds {
            ...
            let vindex = build_data.map.xy_index(vx, vy);
        ...
        for y in 1..build_data.map.height - 1 {
            for x in 1..build_data.map.width - 1 {
                let mut neighbors = 0;
                let my_index = build_data.map.xy_index(x, y);
                let my_seed = voronoi_membership[my_index];

                if voronoi_membership[build_data.map.xy_index(x - 1, y)] != my_seed { neighbors += 1; }
                if voronoi_membership[build_data.map.xy_index(x + 1, y)] != my_seed { neighbors += 1; }
                if voronoi_membership[build_data.map.xy_index(x, y - 1)] != my_seed { neighbors += 1; }
                if voronoi_membership[build_data.map.xy_index(x, y + 1)] != my_seed { neighbors += 1; }

                if neighbors < 2 {
                    build_data.map.tiles[my_index] = TileType::Floor;
                }
            }
