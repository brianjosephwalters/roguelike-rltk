use crate::map_builders::waveform_collapse::constraints::MapChunk;
use crate::Map;
...
impl Solver {
    ...
    pub fn iteration(&mut self, map: &mut Map, rng: &mut super::RandomNumberGenerator) -> bool {
        ...
        // Populate the neighbor count on the remaining list
        let mut remain_copy = self.remaining.clone();
        let mut neighbors_exist = false;
        for r in remain_copy.iter_mut() {
            let index = r.0;
            let chunk_x = index % self.chunks_x;
            let chunk_y = index / self.chunks_x;
        ...
        // Pick a random chunk we haven't dealt with yet, get its index, and remove from remaining list.
        ...
        if chunk_x > 0 {
            let left_index = self.chunk_index(chunk_x - 1, chunk_y);
            match self.chunks[left_index] {
                None => {},
                Some(nt) => {
                    neighbors += 1;
        ...
        if chunk_x < self.chunks_x - 1 {
            let right_index = self.chunk_index(chunk_x + 1, chunk_y);
            match self.chunks[right_index] {
                None => {},
                Some(nt) => {
                    neighbors += 1;
        ...
        if chunk_y > 0 {
            let up_index = self.chunk_index(chunk_x, chunk_y - 1);
            match self.chunks[up_index] {
                None => {}
                Some(nt) => {
                    neighbors += 1;
        ...
        if chunk_y < self.chunks_y - 1 {
            let down_index = self.chunk_index(chunk_x, chunk_y + 1);
            match self.chunks[down_index] {
                None => {},
                Some(nt) => {
                    neighbors += 1;
        ...
        if neighbors == 0 {
            ...
            for y in top_y..bottom_y {
                for x in left_x..right_x {
                    let map_index = map.xy_index(x, y);
            ...
            for new_chunk_index in options_to_check.iter() {
                ...
                for o in options.iter() {
                    if !o.contains(new_chunk_index) { possible = false; }
                }
                if possible {
            ...
            if possible_options.is_empty() {
                ...
                let new_chunk_index = if possible_options.len() == 1 {
                    0
                } else {
                    rng.roll_dice(1, possible_options.len() as i32) - 1
                };

                self.chunks[chunk_index] = Some(new_chunk_index as usize);
                let left_x =  chunk_x as i32 * self.chunk_size as i32;
                let right_x = (chunk_x as i32 + 1) * self.chunk_size as i32;
                let top_y = chunk_y as i32 * self.chunk_size as i32;
                let bottom_y = (chunk_y as i32 + 1) * self.chunk_size as i32;
                ...
                for y in top_y..bottom_y {
                    for x in left_x..right_x {
                        let map_index = map.xy_index(x, y);
                        let tile = self.constraints[new_chunk_index as usize].pattern[i];
                        map.tiles[map_index] = tile;
                        i += 1;
                    }
                }
